  ...
  return (
    <Card className="mt-10">
        <ul> 
            ...
            {workers.map((worker)=>(
                <li className='flex justify-between cursor-pointer hover:shadow-xl p-2 transition-all' key={worker.id}
                onClick={()=> deleteWorker(worker.id)}
                >
                <span>{worker.name}</span>
                <span className='text-red-500 font-medium'>{worker.wage}₺</span>
            </li>
            ))}
        </ul>
    </Card>
